...
class TID2013Dataset(Dataset):
    def __init__(self, root: str, phase: str = "train", crop_size: int = 224, transform=None):
        ...
        # Use reference images as "some_other_image_path"
        self.some_other_image_path = [os.path.join(root, "reference_images", img.split("_")[0] + ".BMP") for img in self.images]

        # Add paths for img_B_ds (assuming it's another reference or distorted image)
        self.img_B_ds_paths = [os.path.join(root, "distorted_images", img) for img in self.images]  # 실제 경로에 따라 조정 필요
        ...
        print("Some other image paths:", self.some_other_image_path[:5])  # 처음 5개 추가 이미지 경로 출력
    ...
    def __getitem__(self, index: int) -> dict:
        img_A_orig = self.load_image(self.image_paths[index])
        img_A_ds = self.load_image(self.reference_paths[index])
        img_B_orig = self.load_image(self.some_other_image_path[index])  # 추가된 img_B_orig
        img_B_ds = self.load_image(self.img_B_ds_paths[index])  # img_B_ds 로드
        mos = torch.tensor(self.mos[index], dtype=torch.float32)

        # Null 체크
        if img_A_orig is None or img_A_ds is None or img_B_orig is None or img_B_ds is None:
